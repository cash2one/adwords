...
for i in range(len(comb2)):
    if ready_dict[int(comb2[i][0])][1] == 'state':
        ...
        kw_ad_group.append("Longtail Retreats %s" %kw_campaign[0].split()[-1])
...
# for i in range(len(camps)):
#     camps[i] = camps[i].split()
#     if '-' in camps[i]: #if the destination is a state
#         dash = camps[i].index('-')
#         groups.append('Longtail Retreats %s' %' '.join(camps[i][dash+1:]))
#     else: #if the destination is a country
#         groups.append('Longtail Retreats %s' %' '.join(camps[i][1:]))
#     camps[i] = ' '.join(camps[i]) #join the items in camps again
